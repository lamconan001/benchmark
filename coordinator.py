...
class BenchmarkCoordinator:
    ...
    def run(self, threads, duration, test_type, report_interval, db_type, db_host, db_port, db_user, db_password, db_name, schema):
        if db_type == 'mysql':
    ...
    def visualize(self, run_ids, sqlite_path=None):
        path = sqlite_path if sqlite_path else self.sqlite_path
        do_visualize(path, run_ids)
